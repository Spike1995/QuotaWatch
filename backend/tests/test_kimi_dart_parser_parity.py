"""Lock the Python Kimi parser to the golden results shared with Dart.

The source payloads and expected values are sanitized repository fixtures.
This test is offline and never reads credentials or contacts Kimi.
"""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any

from app.providers.kimi_parser import parse_kimi_usage

FIXTURE_DIR = Path(__file__).resolve().parents[1] / "fixtures" / "kimi"
EXPECTED_PATH = FIXTURE_DIR / "parity" / "parser_expected.json"


def _normalize(payload: dict[str, Any]) -> list[dict[str, Any]]:
    windows = parse_kimi_usage(payload)
    return [
        {
            "label": window.label,
            "used": window.used,
            "limit": window.limit,
            "unit": window.unit,
            "resetAt": (
                window.reset_at.isoformat().replace("+00:00", "Z")
                if window.reset_at is not None
                else None
            ),
        }
        for window in windows
    ]


def test_python_parser_matches_shared_dart_golden() -> None:
    expected = json.loads(EXPECTED_PATH.read_text(encoding="utf-8"))
    expected.pop("_comment", None)

    actual: dict[str, list[dict[str, Any]]] = {}
    for fixture_name in expected:
        payload = json.loads((FIXTURE_DIR / fixture_name).read_text(encoding="utf-8"))
        payload.pop("_comment", None)
        actual[fixture_name] = _normalize(payload)

    assert actual == expected
